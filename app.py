...
# ============================================================
# 2. CSS GIAO DIỆN (PHÔNG CHỮ KHỔNG LỒ CHO BÁC TÀI)
# ============================================================
st.markdown("""
<style>
/* Dọn dẹp giao diện Streamlit thừa */
#MainMenu, footer, header { visibility:hidden !important; }
.stApp { background:#f4f7f6; }
.block-container { max-width:600px; padding: 1rem 1rem 3rem 1rem !important; }

/* Form nhập liệu: Chữ cực to */
div[data-testid="stTextInput"] label { font-size:20px !important; font-weight:900 !important; color: #1e293b; }
div[data-testid="stTextInput"] input { min-height:60px !important; border-radius:12px !important; font-size:24px !important; font-weight: bold !important; border: 2px solid #cbd5e1; }

/* Nút bấm siêu bự */
div.stButton > button { border-radius:12px !important; font-weight:900 !important; font-size:24px !important; min-height:75px !important; box-shadow: 0 4px 10px rgba(0,0,0,0.1) !important; text-transform: uppercase;}

/* Khung Hoá Đơn */
.receipt-box { border: 4px solid #00A86B; border-radius:16px; padding:20px; text-align:center; background:#fff; margin-bottom:15px; box-shadow:0 8px 20px rgba(0,168,107,.15); }

/* Banner chữ chạy */
.marquee-container { background:#087f4f; color:#fff; padding:12px; border-radius:10px; font-weight:900; font-size:16px; margin-bottom:15px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);}

/* Nút SOS / Zalo dưới cùng */
.btn-sos, .btn-zalo { color:#fff; padding:18px; border-radius:12px; text-align:center; font-weight:900; font-size:20px; text-decoration:none; display:block; margin-bottom: 12px; }
.btn-sos { background:#dc2626; box-shadow: 0 4px 10px rgba(220, 38, 38, 0.3);} 
.btn-zalo { background:#0068ff; box-shadow: 0 4px 10px rgba(0, 104, 255, 0.3);}
.btn-sos:hover, .btn-zalo:hover { opacity: 0.9; color: #fff; }
</style>
...
# ============================================================
# 4. KHẮC PHỤC LỖI VĂNG ĐĂNG NHẬP KHI KẾT THÚC CHUYẾN
# ============================================================
# Xử lý ngay lập tức nếu URL có tín hiệu Kết Thúc (action=stop)
if "action" in st.query_params and st.query_params["action"] == "stop":
    # 1. Phục hồi ngay phiên đăng nhập bằng dữ liệu mang về từ URL (bỏ qua check DB cho mượt)
    st.session_state["logged_in"] = True
    st.session_state["user_phone"] = st.query_params.get("phone", "")
    st.session_state["user_name"] = st.query_params.get("name", "Thành viên")
    ...
    # 4. Xoá rác URL nhưng Giữ Lại Phone & Name để không bao giờ bị văng
...
# ============================================================
# 5. AUTO LOGIN (Cho những lần mở app bình thường)
# ============================================================
...
if not st.session_state["logged_in"]:
    st.markdown("<h1 style='text-align:center; color:#00A86B; font-size:45px;'>🛵 4567 XE ÔM</h1>", unsafe_allow_html=True)
...
# ============================================================
# 7. GIAO DIỆN CHÍNH (ĐÃ ĐƯỢC LÀM GỌN GÀNG VÀ CHỮ TO)
# ============================================================
if st.session_state.get("login_success_effect"):
    st.toast("Đăng nhập thành công!", icon="✅")
    st.session_state["login_success_effect"] = False

# Thanh Header Logo + Tên Bác Tài gọn gàng, rõ ràng
st.markdown(f"""
<div style="display:flex; align-items:center; justify-content:space-between; background:#fff; border:2px solid #00A86B; border-radius:12px; padding:15px; margin-bottom:15px; box-shadow:0 4px 10px rgba(0,168,107,.1);">
    <div style="font-size:24px; font-weight:900; color:#00A86B;">🛵 4567 XE ÔM</div>
    <div style="text-align:right; font-size:16px; line-height:1.4;">Tài xế: <b>{st.session_state['user_name']}</b><br><span style="color:#10b981; font-weight:900;">🟢 SẴN SÀNG</span></div>
</div>
""", unsafe_allow_html=True)

# Banner chữ chạy cảnh báo an toàn giao thông
st.markdown("""
<div class="marquee-container">
    <marquee behavior="scroll" direction="left" scrollamount="6">
        📢 LƯU Ý: Luôn tuân thủ an toàn giao thông • Chúc các bác tài cuốc xe đắt hàng, thượng lộ bình an! 💚
    </marquee>
</div>
""", unsafe_allow_html=True)
...
# ---> TRẠNG THÁI 1: CHỜ KHÁCH (Nhập liệu và Bắt đầu ngay trên đầu)
if not st.session_state.trip_active and not st.session_state.trip_ended_at:
    st.markdown("<div style='text-align:center; color:#087f4f; font-size:22px; font-weight:900; margin-bottom:10px;'>📍 NHẬP KHÁCH MỚI</div>", unsafe_allow_html=True)
...
# ---> TRẠNG THÁI 2: ĐANG CHẠY (Nút kết thúc cùng vị trí với Bắt đầu, cước phí trực quan)
elif st.session_state.trip_active:
    current_start_ts = st.session_state.get('trip_started_at', time.time())
    
    html_live_tracker = f"""
    <div style="background: #f0fdf4; border: 4px solid #22c55e; border-radius: 16px; padding: 25px 15px; text-align: center; margin-bottom: 25px; box-shadow: 0 8px 20px rgba(34, 197, 94, 0.15);">
        <div style="color: #166534; font-size: 24px; font-weight: 900;">💵 CƯỚC PHÍ TẠM TÍNH</div>
        <div id="price" style="color: #15803d; font-size: 70px; font-weight: 900; margin: 15px 0; text-shadow: 2px 2px 4px rgba(0,0,0,0.1);">0 đ</div>
        <div style="color: #334155; font-size: 26px; font-weight: bold;"><span id="km">0.00</span> km <span style="font-size:18px; color:#64748b;">({DONG_GIA:,.0f}đ/km)</span></div>
        <div id="debug_acc" style="font-size: 14px; color: #94a3b8; margin-top: 10px;">Đang tìm vệ tinh GPS...</div>
    </div>
    
    <!-- Nút kết thúc chuyến đặt chính giữa màn hình -->
    <button id="btnStop" onclick="stopTripNow()" style="width: 100%; background: #ef4444; color: white; border: none; border-radius: 12px; padding: 25px; font-size: 26px; font-weight: 900; cursor: pointer; box-shadow: 0 5px 15px rgba(239, 68, 68, 0.4); text-transform: uppercase;">
        🛑 KẾT THÚC CHUYẾN
    </button>
    ...
    function stopTripNow() {{
        ...
        // Gắn theo dữ liệu đăng nhập để không bao giờ bị văng ra ngoài
        ...
        parentUrl.searchParams.set("name", "{st.session_state.get('user_name', '')}");
...
# ---> TRẠNG THÁI 3: KẾT THÚC (Hoá đơn chữ to)
    ...
    st.markdown(
        ...
        <div class="receipt-box">
            <div style="font-size: 30px; font-weight: 900; color: #0f172a; margin-bottom: 10px;">🧾 HOÁ ĐƠN 4567</div>
            <div style="text-align: left; font-size:22px; line-height:1.8; color: #334155;">
                ...
                <div style="font-size:24px; font-weight:bold; text-align:center; color:#64748b;">TỔNG THU:</div>
                <div style="font-size:55px; font-weight:900; color:#10b981; text-align:center; padding: 10px 0;">
                    {fare:,.0f} đ
                </div>
            </div>
        </div>
...
# ============================================================
# 8. KHU VỰC HỖ TRỢ VÀ ĐĂNG XUẤT (DỜI XUỐNG CUỐI CÙNG)
# ============================================================
st.write("---")
st.markdown("<h4 style='text-align:center; color:#64748b; font-weight: 900; margin-bottom:15px;'>HỖ TRỢ BÁC TÀI</h4>", unsafe_allow_html=True)

# Các nút cứu hộ được đưa xuống đáy để gọn màn hình trên
st.markdown('<a href="tel:0978666620" class="btn-sos">🚨 GỌI SOS KHẨN CẤP</a>', unsafe_allow_html=True)
st.markdown('<a href="https://zalo.me/0978666620" class="btn-zalo" target="_blank">💬 NHẮN ZALO TRUNG TÂM</a>', unsafe_allow_html=True)
