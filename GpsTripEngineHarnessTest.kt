...
class GpsTripEngineHarnessTest {
    ...
    private class CurrentV3Engine(
        ...
        fun add(point: GpsPoint) {
            if (
                !point.accuracyM.isFinite() ||
                point.accuracyM <= 0.0 ||
                point.accuracyM > accuracyMaxM
            ) {
                return
            }
            ...
            val dt = max(
                0.5,
                (point.elapsedMs - prev.elapsedMs) / 1000.0
            )
            ...
            if (distance > maxSegmentM || speedToCheck > maxSpeedMps) {
                return
            }
     ...
     *
     * It keeps the last committed anchor until the displacement reaches
     * minMoveM, so many small genuine movements can accumulate without
     * turning every tiny GPS jitter into distance.
     */
    private class CandidateBufferedEngine(
        ...
        fun add(point: GpsPoint) {
            ...
            val dt = max(
                0.5,
                (point.elapsedMs - base.elapsedMs) / 1000.0
            )
            ...
            if (displacement > maxSegmentM || speedToCheck > maxSpeedMps) {
                return
            }

            if (displacement < minMoveM) {
                return
            }
    ...
    fun stationaryJitter_must_not_create_distance() {
        val engine = CandidateBufferedEngine(
            accuracyMaxM,
            minMoveM,
            maxSpeedMps,
            maxSegmentM
        ...
        assertEquals(
            "Standing still with sub-threshold jitter must stay at 0 m",
            0.0,
            engine.totalMeters,
            0.01
        )
    ...
    fun slowWalking_should_accumulate_small_steps() {
        ...
        val candidate = CandidateBufferedEngine(
            accuracyMaxM,
            minMoveM,
            maxSpeedMps,
            maxSegmentM
        ...
        assertEquals(
            "Legacy engine should lose these <3m steps",
            0.0,
            current.totalMeters,
            0.01
        )
...
/**
 * Test-only Haversine distance helper.
 *
 * IMPORTANT:
 * This function is intentionally outside GpsTripEngineHarnessTest so that
 * both nested test engines can resolve it correctly.
 *
 * It does NOT modify production GPS code.
 */
